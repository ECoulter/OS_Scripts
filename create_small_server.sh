...
if [[ -z "$1" ]]; then
  echo "NO openrc.sh given! Please re-run with ./create_small_server.sh <openrc-name> <server-name>"
  exit
fi
...
if [[ -z "$4" ]]; then
  echo "NO SECURITY GROUP GIVEN! Please re-run with ./create_small_server.sh <openrc-name> <server-name>"
  exit
else
	secgroup=$4
fi
...
image_name=$(openstack image list | grep -iE "API-Featured-centos7-[[:alpha:]]{3,4}-[0-9]{2}-[0-9]{4}" | awk '{print $4}')
...
openstack server create --flavor $server_size  --image $image_name --key-name jecoulte-key --security-group $secgroup --nic net-id=${OS_USERNAME}-api-net $2
